...
import com.ems.attendance.service.DepartmentService;
...
public class DepartmentController {
		...
		public ResponseEntity<?> adddepartment(@RequestBody DepartmentDetail departmentdetail) {	
			 HttpHeaders headers = new HttpHeaders();
		        ...
		        try {   	
		    		return null;//ResponseEntity.status(HttpStatus.CREATED).headers(headers).body( departmentservice.adddepartment(departmentdetail));
		    }catch (Exception e) {
		...
		public ResponseEntity<?> findalldepartment() {
			 HttpHeaders headers = new HttpHeaders();
		        ...
		        try {   	
		    		return null;// ResponseEntity.status(HttpStatus.CREATED).headers(headers).body(departmentservice.findalldepartment());
		    }catch (Exception e) {
		    	headers.add("Message", "false");
		    	return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).headers(headers).body("Failed to add the user");
			}
